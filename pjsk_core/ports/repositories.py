...
class UserRepository(Protocol):
    ...
    """Return the existing user for *qq*, creating one (game_id=None) if absent.

    Safe for concurrent callers — uses INSERT OR IGNORE + re-read so
    two simultaneous first-time registrations for the same QQ both
    return the same row.
    """
