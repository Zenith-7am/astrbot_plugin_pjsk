"""ModelScope (魔搭社区) vision engine adapter.

Uses the OpenAI-compatible chat completions endpoint on ModelScope's
free API Inference service.  Images are sent as Base64 Data URLs.

ModelScope free tier: up to 2,000 calls/day total, 500 per model.
Registration + Alibaba Cloud binding required.
"""
...
class ModelScopeVisionEngine:
    """ModelScope free-tier vision engine (OpenAI-compatible).

    Args:
        api_key: ModelScope SDK token from
            https://modelscope.cn/my/access/token
        model: Model ID on ModelScope, e.g.
            ``Qwen/QVQ-72B-Preview``.  Free models are listed at
            https://modelscope.cn/models (filter by API-Inference).
            Check regularly — models may be added or removed.
    ...
    async def recognize(
        ...
        *,
        ...
        """Recognise a PJSK score screenshot using ModelScope vision.
        ...
        Returns:
            Parsed OcrObservation from the ModelScope response.
    ...
    def _parse_response(self, data: dict[str, Any]) -> OcrObservation:
        try:
            ...
            raise VisionResponseError(
                f"Cannot parse ModelScope response: {e}"
            ) from e
